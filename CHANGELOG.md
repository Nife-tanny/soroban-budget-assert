...
- **Local-vs-network cost-gap measurements for four operation types (issues #414–#417).** New `MEASUREMENTS.md` sections, fixtures and measurement tests, each with real local `Env::cost_estimate().budget()` figures and the network `simulateTransaction` column marked pending:
  ...
  - **Cross-contract call depth (#416).** New `RelayContract` fixture (not a change to `cross_contract_test.rs`); test `measure_call_depth_gap.rs` measures depths 1–4. Finding: per-hop cost is constant (~3.62 M CPU) and accumulation is strictly linear; no protocol depth limit hit through depth 4.
  - **Wasm size vs. deploy cost (#417).** New `bloat-contract` crate (real algorithms, not padding) as a mid-size fixture; test `measure_deploy_cost_gap.rs` measures `Env::register` cost for three contracts of 827 / 14,357 / 29,906 bytes. Finding: deploy cost is approximately linear in wasm size (`CPU ≈ 0.47 M + 226·bytes`), not stepped; contract size limit is the `maxContractSizeBytes` ledger setting (Protocol 22 default 64 KiB).
  - Fixture JSON capture records under `cargo-budget-report/fixtures/`, coverage-table rows in `MEASUREMENTS.md`, and `budget.toml` entries for the network capture. `.github/workflows/budget.yml` now builds `host-function-contract` and `bloat-contract` wasm alongside `amm-pool-contract`. The `read_bytes` (30k → 36k) and cross-contract macro (350M → 420M) ceilings in `budget_test.rs` / `cross_contract_test.rs` move to absorb the ~4.5 KB the new fixtures add to `amm_pool_contract.wasm`, per the same rationale under which the `noop` export was kept.
- **Typed function-argument specs in `budget.toml` (issue #152).** `[functions.<name>].args` entries may now be tables — `{ name, type, value }` — as well as bare strings, so an entry point taking an address, symbol, bytes or a structured (`json`) value can be simulated, not just `do_expensive_work(n: u32)`. `type = "address"` supports `generate = true` for a deterministic dummy strkey. An unknown type or missing value is a named error, never a silent skip. Bare-string lists are unchanged. Schema documented in `docs/src/reference.md`.
...
- **Budget attributes apply to `impl` blocks (issue #393).** `#[budget_cpu_lt]`, `#[budget_mem_lt]`, `#[budget_write_bytes_lt]`, `#[budget_read_bytes_lt]` and `#[budget_lt]` on an `impl` block instrument every method in it against the same limit. A method carrying its own `#[budget_*]` attribute is governed by that instead (per-method override, and the opt-out path for a helper that should not be asserted). Failure messages gain a `` [fn `name`] `` tag naming the offending method; the single-function message is byte-identical to before. `#[budget_scaling]`, modules and traits stay unsupported and still fail with a compile error, as does an `impl` block with no instrumentable method. New `tests/ui/pass/pass_impl_block.rs` and `tests/ui/on_empty_impl.rs` fixtures; documented in `docs/src/reference.md` and `docs/src/macro_architecture.md`.
- **`#[budget_read_bytes_lt]` UI-test and reference coverage completed (issue #388).** New `tests/ui/pass/pass_read_bytes_baseline.rs` (marginal-cost subtraction, including the below-baseline saturation case) and `tests/ui/read_bytes_no_arg.rs` (the no-argument rejection path), plus the `config = "KEY"`, `baseline = EXPR`, and failure-message-format details filled into the `docs/src/reference.md` section.
...
- **Literal `env_file` paths on the budget macros are now checked at macro-expansion time (issue #392).** `#[budget_cpu_lt(env_file = "PATH", env = "VAR")]` with a string-literal `PATH` that resolves to no file is a compile error naming the path, instead of a runtime panic. Non-literal paths (`env_file = CONST`) may be build-generated and are still resolved at test runtime. Every existing usage passes a `const` and is unaffected. `docs/src/reference.md` gains a "Limit sources and their precedence" section documenting the resolution order.
